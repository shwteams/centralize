...
if (isset($_GET["task"])) {
    ...
    else if ($task == "getAllProduction"){
        $str_PRODUCTION_ID = "";
        if(isset($_GET['str_PRODUCTION_ID']))
        {
            $str_PRODUCTION_ID = htmlentities($_GET['str_PRODUCTION_ID']);
        }
        getAllProduction($str_PRODUCTION_ID, $db);
    }
    else if ($task == "deleteProduction"){
        $str_PRODUCTION_ID = $_GET["str_PRODUCTION_ID"];
        deleteProduction($str_PRODUCTION_ID, $db);
    }
...
else if (isset($_POST['editProduction'])) {
    $str_PRODUCTION_ID = htmlentities(trim($_POST['str_PRODUCTION_ID']));
    $str_CONTRACT_ID = htmlentities(trim($_POST['str_CONTRACT_ID']));
    $str_INTERMEDIATE_ID = htmlentities(trim($_POST['str_INTERMEDIATE_ID']));
    $dt_EFFET = htmlentities(trim($_POST['dt_EFFET']));
    $dt_ECHEANCE = htmlentities(trim($_POST['dt_ECHEANCE']));
    ...
    $int_MAJO = htmlentities(trim($_POST['int_MAJO']));
    //exit();
    editProduction($str_PRODUCTION_ID, $str_CONTRACT_ID, $str_INTERMEDIATE_ID, $dt_EFFET, $dt_ECHEANCE, $int_PRIME_TTC, $int_COMMISSION, $int_PRIME_NET, $int_TAUX_COMMISSION, $int_AVENANT, $int_MAJO, $db);
}