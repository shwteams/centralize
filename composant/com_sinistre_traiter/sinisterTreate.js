var url = "composant/com_sinistre_traiter/controlerSinisterTreate.php";
var datatable = "";
$(function () {
    getAllSinisterTreat("");
    ...
    $('#modal_edit_key').submit(function (e) {
        ...
        //var mt_PROVISION_FIN = $('#modal_edit_key #mt_PROVISION_FIN_EDIT').val();

        if (str_NUMERO_SINISTRE == "" || str_SINISTER_ID == "" || str_CONTRAT_ID == "" || dt_SURVENANCE == "" || dt_TRAITEMENT == "" || mt_EVAL == "" || mt_CUMULE_PAYE == ""  || mt_PROVISION == "" ) {
            swal({
                title: "Echec",
            ...
            editeSinister(str_SINISTER_ID, str_CONTRAT_ID, dt_SURVENANCE, dt_TRAITEMENT, mt_EVAL, mt_CUMULE_PAYE, mt_PROVISION);
        }
    ...
    $('#modal_editing_key').submit(function (e) {
        ...
        //var mt_PROVISION_FIN = $('#modal_editing_key #mt_PROVISION_FIN_EDITING').val();
    ...
    $('#add_key_form').submit(function (e) {
        ...
        var mt_CUMULE_PAYE = $('#add_key_form #mt_CUMULE_PAYE').val();
        var mt_PROVISION = $('#add_key_form #mt_PROVISION').val();
        //var mt_PROVISION_FIN = $('#add_key_form #mt_PROVISION_FIN').val();

        if (str_NUMERO_SINISTRE == "" || str_CONTRAT_ID == "" || dt_SURVENANCE == "" || dt_TRAITEMENT == "" || mt_EVAL == "" || mt_CUMULE_PAYE == "" || mt_PROVISION == ""  ) {
            swal({
                title: "Echec",
            ...
            addSinister(str_CONTRAT_ID, dt_SURVENANCE, dt_TRAITEMENT, mt_EVAL, mt_CUMULE_PAYE, mt_PROVISION);
        }
    });
...
function getAllSinisterTreat(str_SINISTER_ID){
    var task = "getAllSinisterTreat";
        ...
        if (obj[0].code_statut == "1")
        {
            ...
            if (obj[0].results.length > 0)
            {
                $.each(results, function (i, value)//
                {
                    ...
                    var td_ADMINISTRATOR = $('<td class="column-data-table">' + results[i].str_NOM + '</td>');
                    ...
                    var td_DERNIERE_MODIFICATION = $('<td class="column-data-table">' + results[i].dt_UPDATED + '</td>');
                    ...
                    //var td_PROVISION_FIN = $('<td class="column-data-table">' + results[i].mt_PROVISION_FIN + '</td>');
                    var td_BONI = $('<td class="column-data-table">' + results[i].mt_BONI + '</td>');
                    var td_MALI = $('<td class="column-data-table">' + results[i].mt_MALI + '</td>');
                    var td_BONIPOTENTIEL = $('<td class="column-data-table">' + results[i].mt_BONI_POTENTIEL + '</td>');
                    var td_AUTRE_OBSERVATION = $('<td class="column-data-table">' + results[i].str_AUTRE_OBSERVATION + '</td>');
                    var td_ETAT = $('<td class="column-data-table">' + results[i].str_LIBELLE + '</td>');
                    ...
                    var btn_editing = $('<span class=" btn-action-custom btn-action-edit" id="modal_editing_key" data-toggle="modal"  title="Traiter"><i class="fa fa-edit"></i> </span> ').click(function () {
...
function editeSinister(str_SINISTER_ID, str_CONTRAT_ID, dt_SURVENANCE, dt_TRAITEMENT, mt_EVAL, mt_CUMULE_PAYE, mt_PROVISION) {
    $.ajax({
        url: url, // La ressource ciblée
        type: 'POST', // Le type de la requête HTTP.
        data: 'editeSinister=editeSinister&str_SINISTER_ID='+str_SINISTER_ID+'&str_CONTRAT_ID='+str_CONTRAT_ID+'&dt_SURVENANCE='+dt_SURVENANCE+'&dt_TRAITEMENT='+dt_TRAITEMENT+'&mt_EVAL='+mt_EVAL+'&mt_CUMULE_PAYE='+mt_CUMULE_PAYE+'&mt_PROVISION='+mt_PROVISION,
        dataType: 'text',
        success: function (response) {
            //alert(json);return;
            var obj = $.parseJSON(response);
