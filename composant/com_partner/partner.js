var url = "composant/com_partner/controlerPartner.php";
var datatable = "";
$(function () {
    getAllPartner("");
    ...
    $('#modal_edit_key').submit(function (e) {
        ...
        var str_PARTENAIRE_ID = $('#modal_edit_key #str_PARTENAIRE_ID').val();
        ...
        if (str_LIBELLE == "") {
            ...
            editePartner(str_PARTENAIRE_ID, str_LIBELLE);
        }
    ...
    $('#add_key_form').submit(function (e) {
        e.preventDefault();haveSession();
        var str_PARTENAIRE_ID = $('#add_key_form #str_PARTENAIRE_ID').val();
        var str_LIBELLE = $('#add_key_form #str_LIBELLE').val();
        if (str_PARTENAIRE_ID == "" || str_LIBELLE == "" ) {
            swal({
                title: "Echec",
            ...
            addPartner(str_PARTENAIRE_ID, str_LIBELLE);
        }
    });
...
function getAllPartner(str_PARTENAIRE_ID){
    var task = "getAllPartner";
    
    $.get(url+"?task="+task+"&str_PARTENAIRE_ID="+str_PARTENAIRE_ID, function(json, textStatus){
        ...
        if (obj[0].code_statut == "1")
        {
            ...
            if (obj[0].results.length > 0)
            {
                ...
                {
                    var tr = $('<tr class="line-data-table" id="' + results[i].str_PARTENAIRE_ID + '"></tr>');
                    ...
                    var btn_edit = $('<span class=" btn-action-custom btn-action-edit" id="modal_edit_key" data-toggle="modal"  title="Modifier"><i class="fa fa-edit"></i> | </span> ').click(function () {
                        ...
                        //alert(id_key);return;
                        $('#edit-key-form #str_PARTENAIRE_ID').val(id_key);
                        getKeyById(id_key);
                    });
...
function addPartner(str_PARTENAIRE_ID, str_LIBELLE) {
    $.ajax({
        url: url, // La ressource ciblée
        type: 'POST', // Le type de la requête HTTP.
        data: 'addPartner=addPartner&str_LIBELLE=' + str_LIBELLE,
        dataType: 'text',
...
function editePartner(str_PARTENAIRE_ID, str_LIBELLE) {
    $.ajax({
        url: url, // La ressource ciblée
        type: 'POST', // Le type de la requête HTTP.
        data: 'editePartner=editePartner&str_LIBELLE_EDIT=' + str_LIBELLE + '&str_PARTENAIRE_ID='+str_PARTENAIRE_ID,
        dataType: 'text',
        success: function (response) {
            //alert(json);return;
            var obj = $.parseJSON(response);
...
function deletePartner(str_PARTENAIRE_ID) {
    //alert(str_PARTENAIRE_ID);
    $.ajax({
        url: url, // La ressource ciblée
        type: 'GET', // Le type de la requête HTTP.
        data: 'task=deletePartner&str_PARTENAIRE_ID=' + str_PARTENAIRE_ID,
        dataType: 'text',
        success: function (response) {
            var obj = $.parseJSON(response);
            if (obj[0].code_statut == "1")
...
function getKeyById(str_PARTENAIRE_ID)
{
    //alert(lg_OFFRE_ID);
    var task = "getAllPartner";
    $.get(url + "?task=" + task + "&str_PARTENAIRE_ID=" + str_PARTENAIRE_ID, function (json, textStatus)
    {
        ...
        if (obj[0].code_statut == "1")
        {
            ...
            if (obj[0].results.length > 0)
            {
                $.each(results, function (i, value)
                {
                    $('#modal_edit_key #str_PARTENAIRE_ID').val(results[i].str_PARTENAIRE_ID);
